...
//     // 숫자 입력값인 경우 음수를 0으로 변경
//     if (name === "productPrice" || name === "productQuantity") {
//       const numericValue = Math.max(Number(value), 0); // 음수인 경우 0으로 변경
//       value = numericValue.toString(); // 음수인 경우 0으로 변경
//     }
...
function ProductUpload() {
  ...
  const handleSaveProduct = async (
    ...
    await checkAndRefreshToken(); // API 호출 전에 액세스 토큰 체크 및 갱신
  ...
  const deleteUploadedImages = async () => {
    try {
      ...
      // product.productImages 배열의 각 요소를 순회하며 DELETE 요청 보내기
